...
router.patch('/:id', async (req, res) => { //Aqui estamos definiendo la arrow function como asincrona
  try {
  const { id } = req.params
  const body = req.body
  ...
  res.json(product)
    ...
    res.status(404).json({message: error.message}) //!Asi es como enviamos el mensaje de error
  }
